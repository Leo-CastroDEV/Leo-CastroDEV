<h1 align="center">👋 Olá, eu sou Leonardo Castro</h1>

<p align="center">
  <strong>Desenvolvedor Full Stack em formação | Análise e Desenvolvimento de Sistemas</strong>
</p>

<p align="center">
  <a href="https://github.com/Leo-CastroDEV">
    <img src="https://img.shields.io/badge/GitHub-Leo--CastroDEV-181717?style=for-the-badge&logo=github" />
  </a>&nbsp;&nbsp;&nbsp;&nbsp;
  <a href="https://www.linkedin.com/in/leocastrodev/">
    <img src="https://img.shields.io/badge/LinkedIn-Leonardo%20Castro-0A66C2?style=for-the-badge&logo=linkedin" />
  </a>&nbsp;&nbsp;&nbsp;&nbsp;
  <a href="https://www.instagram.com/devleocastro/">
    <img src="https://img.shields.io/badge/Instagram-@devleocastro-E4405F?style=for-the-badge&logo=instagram" />
  </a>
</p>

---

## 🚀 Sobre mim

Sou estudante de **Análise e Desenvolvimento de Sistemas**, apaixonado por tecnologia e desenvolvimento de software.

Tenho experiência prática na construção de **aplicações web, APIs REST, interfaces responsivas e sistemas integrados a bancos de dados**, buscando sempre transformar conhecimento em projetos reais.

Atualmente, estou direcionando minha carreira para o desenvolvimento **Full Stack**, aprimorando conhecimentos em desenvolvimento de interfaces, back-end, bancos de dados, APIs e boas práticas de engenharia de software.

```text
🎓 Análise e Desenvolvimento de Sistemas
💻 Foco: Desenvolvimento Web e Full Stack
🚀 Construindo projetos práticos e aplicações reais
🧠 Interesse: Software, APIs, Banco de Dados e Inteligência Artificial
🌱 Evolução contínua através de projetos e estudos
🎯 Objetivo: Atuar profissionalmente como Desenvolvedor de Software
```

---

## 💼 O que eu desenvolvo

```text
▸ Aplicações Web
▸ APIs REST
▸ Sistemas CRUD
▸ Integração Front-end + Back-end
▸ Sistemas com Banco de Dados
▸ Interfaces responsivas
▸ Projetos com autenticação
▸ Integração com APIs externas
▸ Aplicações utilizando Inteligência Artificial
```

Meu objetivo não é apenas aprender tecnologias, mas **entender como utilizá-las para construir soluções funcionais, escaláveis e úteis**.

---

## 🛠️ Tecnologias & Ferramentas

### Front-end

<p>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/html5/html5-original.svg" height="40" alt="HTML5"/>
<img width="10"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/css3/css3-original.svg" height="40" alt="CSS3"/>
<img width="10"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/javascript/javascript-original.svg" height="40" alt="JavaScript"/>
<img width="10"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/typescript/typescript-original.svg" height="40" alt="TypeScript"/>
<img width="10"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/react/react-original.svg" height="40" alt="React"/>
<img width="10"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/tailwindcss/tailwindcss-original.svg" height="40" alt="Tailwind CSS"/>
<img width="10"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/bootstrap/bootstrap-original.svg" height="40" alt="Bootstrap"/>
</p>

### Back-end

<p>
<img src="https://skillicons.dev/icons?i=nodejs" height="40" alt="Node.js"/>
<img width="10"/>
<img src="https://skillicons.dev/icons?i=express" height="40" alt="Express"/>
<img width="10"/>
<img src="https://skillicons.dev/icons?i=python" height="40" alt="Python"/>
<img width="10"/>
<img src="https://skillicons.dev/icons?i=c" height="40" alt="C"/>
<img width="10"/>
<img src="https://cdn.simpleicons.org/c++/00599C" height="40" alt="C++"/>
</p>

### Banco de Dados

<p>
<img src="https://skillicons.dev/icons?i=mysql" height="40" alt="MySQL"/>
<img width="10"/>
<img src="https://skillicons.dev/icons?i=mongodb" height="40" alt="MongoDB"/>
</p>

### Ferramentas & Deploy

<p>
<img src="https://skillicons.dev/icons?i=git" height="40" alt="Git"/>
<img width="10"/>
<img src="https://skillicons.dev/icons?i=github" height="40" alt="GitHub"/>
<img width="10"/>
<img src="https://skillicons.dev/icons?i=vercel" height="40" alt="Vercel"/>
<img width="10"/>
<img src="https://skillicons.dev/icons?i=postman" height="40" alt="Postman"/>
</p>

---

## ⭐ Projetos em destaque

### 📋 Gerenciador de Tarefas

Aplicação para gerenciamento de tarefas desenvolvida com arquitetura **Front-end + API + Banco de Dados**.

**Principais recursos:**

* Criação e gerenciamento de tarefas
* Edição e exclusão
* Conclusão de tarefas
* Organização por prioridades
* Calendário
* API REST
* Integração com banco de dados
* Interface responsiva

**Stack:** React • JavaScript • Node.js • Express • MySQL

🔗 **Demo:** https://gerenciador-de-tarefas-rho-ten.vercel.app/

---

### 🤖 FocoAI — API para gerenciamento

API desenvolvida para gerenciamento de tarefas e integração com aplicações web.

**Conceitos trabalhados:**

* API REST
* Rotas HTTP
* CRUD
* Autenticação
* Banco de dados
* Integração Front-end / Back-end
* Testes de API

🔗 **API:** https://focoai.onrender.com/

---

### 🌐 Portfólio Devstarter

Meu portfólio profissional foi desenvolvido com uma identidade visual inspirada no conceito **Cyber Tech**, utilizando elementos neon, animações e uma experiência de navegação moderna.

O projeto também conta com integração de **Inteligência Artificial** para interação com visitantes e recursos de contato.

**Stack:** HTML • CSS • JavaScript • Node.js • Express • Bootstrap • Gemini API

🔗 **Portfólio** http://leonardocastro.tech

---

## 📊 Estatísticas do GitHub

<p align="center">
  <img height="140" src="https://github-readme-stats.shion.dev/api?username=Leo-CastroDEV&theme=transparent&hide_border=true&hide_rank=true" />
  <img height="140" src="https://streak-stats.demolab.com/?user=Leo-CastroDEV&theme=transparent&hide_border=true" />
  <img height="140" src="https://github-readme-stats.shion.dev/api/top-langs/?username=Leo-CastroDEV&theme=transparent&hide_border=true&layout=compact" />
</p>


---

## 🐍 Minha atividade

<p align="center">
  <img src="https://raw.githubusercontent.com/Leo-CastroDEV/Leo-CastroDEV/output/pacman-contribution-graph-dark.svg" alt="Pacman Contribution Graph"/>
</p>

---

## 🎯 Atualmente

Estou focado em evoluir principalmente nas seguintes áreas:

```text
▸ Desenvolvimento Full Stack
▸ Arquitetura de APIs
▸ Banco de Dados
▸ React e TypeScript
▸ Node.js
▸ Python
▸ Engenharia de Software
▸ Inteligência Artificial aplicada ao desenvolvimento
▸ Boas práticas e arquitetura de software
```

---

## 🤝 Vamos conversar?

Estou aberto a **oportunidades de estágio, posições júnior, projetos e networking** na área de desenvolvimento de software.

Se você está recrutando desenvolvedores ou quer trocar ideias sobre tecnologia, fique à vontade para entrar em contato.

<p align="center">
  <a href="https://www.linkedin.com/in/leocastrodev/">
    <img src="https://img.shields.io/badge/LinkedIn-Conecte--se%20comigo-0A66C2?style=for-the-badge&logo=linkedin"/>
  </a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
  <a href="https://github.com/Leo-CastroDEV">
    <img src="https://img.shields.io/badge/GitHub-Veja%20meus%20projetos-181717?style=for-the-badge&logo=github"/>
  </a>
</p>

---

<p align="center">
  <strong>💡 Transformando aprendizado em projetos e projetos em experiência.</strong>
</p>

<p align="center">
  <i>Construindo hoje as habilidades que quero usar profissionalmente amanhã.</i>
</p>
